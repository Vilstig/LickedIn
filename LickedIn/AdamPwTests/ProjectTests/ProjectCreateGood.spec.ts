...
test('test', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Nazwa Projektu' }).fill('Moj projekt');
    await page.getByLabel('Kierownik').selectOption('1');
    await page.getByRole('textbox', { name: 'Data Końca' }).fill('2026-02-19');
    await page.getByRole('textbox', { name: 'np. Senior Backend Dev' }).click();
    await page.getByRole('textbox', { name: 'np. Senior Backend Dev' }).fill('Deweolpoer');
